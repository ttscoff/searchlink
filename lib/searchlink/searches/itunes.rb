module SL
  class SearchLink
    def itunes(search_type, search_terms)
      case search_type
      ...
      when /^imov?$/ # iTunes movie search
        dev = false
        url, title = search_itunes('movie', search_terms, dev, @cfg['itunes_affiliate'])
      when /^ipod$/
      ...
      when /^itud?$/ # iTunes app search
        dev = search_type =~ /d$/
        url, title = search_itunes('iPadSoftware', search_terms, dev, @cfg['itunes_affiliate'])
      when /^masd?$/ # Mac App Store search (mas = itunes link, masd = developer link)
        dev = search_type =~ /d$/
        url, title = search_itunes('macSoftware', search_terms, dev, @cfg['itunes_affiliate'])
      end
    ...
    def search_itunes(entity, terms, dev, aff = nil)
      ...
      url = URI.parse("http://itunes.apple.com/search?term=#{ERB::Util.url_encode(terms)}&country=#{@cfg['country_code']}&entity=#{entity}&limit=1")
