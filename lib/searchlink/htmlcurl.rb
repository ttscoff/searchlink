...
class HTMLCurl
  attr_reader :url, :code, :headers, :meta, :links, :head, :body, :source, :title, :description, :body_links, :body_images
  ...
  def extract(before, after)
    before = /#{Regexp.escape(before)}/ unless before.class == Regexp
    after = /#{Regexp.escape(after)}/ unless after.class == Regexp
    @body.scan(/#{before}(.*?)#{after}/)
  end
  ...
  def content_tags(content)
    ...
    res = content.to_enum(:scan, %r{<(?<tag>[^/][a-z0-9]+)(?<attrs> [^>]+)?(?: */>|>(?<content>.*?)</\k<tag>>)}mi).map { Regexp.last_match }
    res.map do |tag|
      if tag['attrs'].nil?
        attrs = nil
      else
        attrs = tag['attrs'].strip.to_enum(:scan, /(?<key>[@a-z0-9-]+)(?:=(?<quot>["'])(?<value>[^"']+)\k<quot>|[ >])?/i).map { Regexp.last_match }
